...
auth.onAuthStateChanged(async user => {
    if (user) {
        ...
        buildNav();
        navigateTo(new URLSearchParams(location.search).get('tab') || 'dashboard');
        updateUI();
        detectDeviceInfo();
...
function renderPageExtension(area, extName) {
    ...
    s.src = `/static/js/extensions/${extName}.js?v=3.4.2`;
...
function renderExtPage(area, extId) {
    ...
    script.src = `/static/js/extensions/${pageType}.js?v=3.4.2`;
    script.onload = () => {
        if (typeof window[renderFn] === 'function') {
            window[renderFn](area, ext);
        } else {
